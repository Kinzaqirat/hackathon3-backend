"""
Enhanced Kafka service with optional Dapr integration for event streaming
"""
...
class EnhancedKafkaService:
    """Enhanced Kafka event streaming service with optional Dapr integration"""
    ...
    async def publish_event(cls, topic: str, event: Dict[str, Any], key: str = None):
        """Publish an event to Kafka with optional Dapr fallback/publishing"""
        try:
            # Try Kafka first
            ...
            if producer is not None:
                ...
                logger.debug(f"Event published to Kafka topic {topic}: {key}")
            else:
                logger.debug(f"Kafka disabled, skipping event to {topic}")
            
            # If Dapr is available, also publish the event there
            if DAPR_AVAILABLE:
                try:
                    DaprService.publish_event(topic, event)
                    logger.debug(f"Event also published via Dapr to topic {topic}")
                except Exception as dapr_error:
                    logger.warning(f"Dapr publish failed: {str(dapr_error)}")
    ...
    @classmethod
    async def get_state(cls, key: str, store_name: str = "learnflow-statestore"):
        """Get state using Dapr if available, otherwise return None"""
        if DAPR_AVAILABLE:
            try:
                return DaprService.get_state(key, store_name)
            except Exception as e:
                logger.error(f"Error getting state for key {key}: {str(e)}")
                return None
        else:
            logger.warning("Dapr not available for state management")
            return None

    @classmethod
    async def save_state(cls, key: str, value: Any, store_name: str = "learnflow-statestore"):
        """Save state using Dapr if available"""
        if DAPR_AVAILABLE:
            try:
                DaprService.save_state(key, value, store_name)
            except Exception as e:
